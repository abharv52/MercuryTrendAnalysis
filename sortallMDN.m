%% MDNsort
function[MDNsort,MDNdy,MDNpre,MDNcon,MDNdep,MDNmpre,MDNmcon,MDNmdep] = sortallMDN(MDN)
D = table2array(MDN(:,2));
Date = cellfun(@datenum, D);
[HGPRE,HGCON,HGDEP] = cleandata(MDN);

MDNsort = sortMDN(Date,HGPRE,HGCON,HGDEP);
[MDNdy] = dayavg(MDNsort);
[monthmed, MDNmpre,MDNmcon,MDNmdep] = monmed(MDNdy);
...
for t = 1:55;
    ...
    if p == 7;
        ...
        MDNdy(d:d+6,l) = MDNsort(t,l)/7;
        MDNdy(d:d+6,i) = MDNsort(t,i)/7;
        MDNdy(d:d+6,i+1) = MDNsort(t,i+1)/7;
    elseif p == 6;
        MDNdy(d:d+5,m) = MDNsort(t,m);
        MDNdy(d:d+5,l) = MDNsort(t,l)/6;
        MDNdy(d:d+5,i) = MDNsort(t,i)/6;
        MDNdy(d:d+5,i+1) = MDNsort(t,i+1)/6;
    elseif p == 8;
        MDNdy(d:d+7,m) = MDNsort(t,m);
        MDNdy(d:d+7,l) = MDNsort(t,l)/8;
        MDNdy(d:d+7,i) = MDNsort(t,i)/8;
...
%% Monthavg
function[monthmed,MDNmpre,MDNmcon,MDNmdep] = monmed(MDNsort)
    %Takes MDNsorted data and averages all the data by month
    monthmed = zeros(12,57);
    temp1 = zeros(31,57);
    temp2 = zeros(31,57);
    temp3 = zeros(31,57);
    temp4 = zeros(31,57);
...
j = j+3;
    ...
    monthmed(1,:) = nanmedian(temp1);
    monthmed(2,:) = nanmedian(temp2);
    monthmed(3,:) = nanmedian(temp3);
    monthmed(4,:) = nanmedian(temp4);
    monthmed(5,:) = nanmedian(temp5);
    monthmed(6,:) = nanmedian(temp6);
    monthmed(7,:) = nanmedian(temp7);
    monthmed(8,:) = nanmedian(temp8);
    monthmed(9,:) = nanmedian(temp9);
    monthmed(10,:) = nanmedian(temp10);
...
j = j+3;
    ...
    monthavg(1,:) = nanmean(temp1);
    monthavg(2,:) = nanmean(temp2);
    monthavg(3,:) = nanmean(temp3);
    monthavg(4,:) = nanmean(temp4);
    monthavg(5,:) = nanmean(temp5);
    monthavg(6,:) = nanmean(temp6);
    monthavg(7,:) = nanmean(temp7);
    monthavg(8,:) = nanmean(temp8);
    monthavg(9,:) = nanmean(temp9);
    monthavg(10,:) = nanmean(temp10);
    monthavg(11,:) = nanmean(temp11);
    monthavg(12,:) = nanmean(temp12);
